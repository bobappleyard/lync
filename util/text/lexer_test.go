...
func TestLexer(t *testing.T) {
	...
	yieldToken := func(id int) func(start int, text string) Token {
		return func(start int, text string) Token {
			return Token{ID: id, Start: start, Text: text}
		}
	}
...
func TestTypedLexer(t *testing.T) {
	p := &Lexer[testToken]{
		...
		finalStates: []finalState[testToken]{
			{Given: 4, Then: func(start int, text string) testToken {
				return identifier{name: text}
			}},
			{Given: 7, Then: func(start int, text string) testToken {
				value, _ := strconv.Atoi(text)
				return literal[int]{value: value}
			}},
			{Given: 14, Then: func(start int, text string) testToken {
				value, _ := strconv.ParseFloat(text, 64)
				return literal[float64]{value: value}
			}},
			{Given: 15, Then: func(start int, text string) testToken {
				return sep{}
			}},
		},
		maxState: 16,
